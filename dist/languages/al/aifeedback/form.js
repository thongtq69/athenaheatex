$(function(){
	...
	$('#pagetitle').val(document.title);
        var crmValidStr = "هذه الخانة  مطلوبة ";
        ...
        $(".crm-form form").submit(function(){
            ...
            var url = "/aifeedback/save.php";
            $.post(url, form.serialize(), function(result){
                form.find(".create-form-submit").removeAttr("disabled").find(".crm-submit-load").remove();
                if(result=='1'){
                    _crmAlertText(1,'تم تنفيذ إرسال البريد الإلكتروني');
                    form[0].reset();
                }
                else if(result=='2'){
    ...
    function _crmAlertText(type,text){
        ...
        var succ = '<div id="crmMailMask"><div class="crmMailMask-box"><div class="crmMailMask-boxTop">'+text+'</div><div class="crmMailMask-boxBot"><button type="button" class="crmMailMask-close"> حسنا </button></div></div></div>';
        if(type==1){

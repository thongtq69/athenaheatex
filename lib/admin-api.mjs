...
export function normalizeResource(resource, input = {}, current = {}) {
  ...
  if (resource === 'pages') {
    ...
    if (!title || !sourceHtml || sourceHtml.length > 2_000_000) throw new Error('Tiêu đề và HTML trang là bắt buộc (tối đa 2 MB).');
  ...
  if (resource === 'sections') {
    ...
    if ((!validPath(pagePath) && pagePath !== '*') || !name || !selector || sourceHtml.length > 1_000_000) throw new Error('Trang, tên, CSS selector và HTML section hợp lệ là bắt buộc.');
  ...
  if (resource === 'categories') {
    ...
    if (!image || !validImage(image)) throw new Error('Ảnh đại diện là bắt buộc: hãy nhập URL http(s), đường dẫn nội bộ hoặc upload một file ảnh.');
    return { ...base, name, path, parentId: text(input.parentId ?? base.parentId, 100), descriptionHtml: normalizeFragmentHtml(String(input.descriptionHtml ?? base.descriptionHtml ?? '').slice(0, 1_000_000), name), image, kind: input.kind === 'service' ? 'service' : 'product', enabled: bool(input.enabled, base.enabled), sortOrder: order(input.sortOrder ?? base.sortOrder) };
  }
  if (resource === 'banners') {
    const title = text(input.title ?? base.title, 500);
    const image = text(input.image ?? base.image, 2000);

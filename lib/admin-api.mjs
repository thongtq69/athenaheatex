...
export function normalizeResource(resource, input = {}, current = {}) {
  ...
  if (resource === 'pages') {
    const path = text(input.path ?? base.path, 500);
    const title = text(input.title ?? base.title, 500);
    const html = String(input.html ?? base.html ?? '');
    if (!validPath(path)) throw new Error('Đường dẫn trang phải bắt đầu bằng / và kết thúc bằng .html.');
    if (!title || !html || html.length > 2_000_000) throw new Error('Tiêu đề và HTML trang là bắt buộc (tối đa 2 MB).');
  ...
  if (resource === 'sections') {
    ...
    const selector = text(input.selector ?? base.selector, 300);
    const html = String(input.html ?? base.html ?? '');
    try { loadSelector(selector); } catch { throw new Error('CSS selector của section không hợp lệ.'); }
    if ((!validPath(pagePath) && pagePath !== '*') || !name || !selector || html.length > 1_000_000) throw new Error('Trang, tên, CSS selector và HTML section hợp lệ là bắt buộc.');
  ...
  if (resource === 'products' || resource === 'services') {
    ...
    const path = text(input.path ?? base.path ?? `/${slug(name)}.html`, 500);
    const image = text(input.image ?? base.image, 2000);
    if (!name || !validPath(path)) throw new Error('Tên và đường dẫn hợp lệ là bắt buộc.');
    if (!validImage(image)) throw new Error('Ảnh phải là URL http(s) hoặc đường dẫn nội bộ bắt đầu bằng /.');
    return { ...base, name, path, categoryId: text(input.categoryId ?? base.categoryId, 100), summary: text(input.summary ?? base.summary, 3000), descriptionHtml: String(input.descriptionHtml ?? base.descriptionHtml ?? '').slice(0, 1_000_000), image, seo: normalizeSeo(input.seo ?? base.seo), enabled: bool(input.enabled, base.enabled), sortOrder: order(input.sortOrder ?? base.sortOrder) };
  }
  if (resource === 'categories') {
    const name = text(input.name ?? base.name, 500);
    const path = text(input.path ?? base.path ?? `/${slug(name)}.html`, 500);
    const image = text(input.image ?? base.image, 2000);
    if (!name || !validPath(path) || !validImage(image)) throw new Error('Tên, đường dẫn và ảnh danh mục không hợp lệ.');
    return { ...base, name, path, parentId: text(input.parentId ?? base.parentId, 100), descriptionHtml: String(input.descriptionHtml ?? base.descriptionHtml ?? '').slice(0, 1_000_000), image, kind: input.kind === 'service' ? 'service' : 'product', enabled: bool(input.enabled, base.enabled), sortOrder: order(input.sortOrder ?? base.sortOrder) };
  }
  if (resource === 'banners') {
    const title = text(input.title ?? base.title, 500);
    const image = text(input.image ?? base.image, 2000);
  ...
  if (resource === 'media') {
    ...
    return { ...base, name, url, alt: text(input.alt ?? base.alt, 500), sourceType: /^https?:\/\//i.test(url) ? 'external' : 'local', enabled: bool(input.enabled, base.enabled), sortOrder: order(input.sortOrder ?? base.sortOrder) };
  }
...
function normalizeSettings(input = {}, current = {}) {
  const value = {
    ...
    copyright: text(input.copyright ?? current.copyright, 500), defaultSeo: normalizeSeo(input.defaultSeo ?? current.defaultSeo), updatedAt: new Date(),
  };
...
export async function handleAdminApi(req, res, url, readJson) {
  ...
  if (action === 'dashboard' && method === 'GET') {
    const [counts, seed] = await Promise.all([cmsCounts(), db.collection(CMS.meta).findOne({ _id: 'website-import-v1' })]);
    return send(res, 200, { counts, seed: cleanDocument(seed) });
  }
  if (action === 'settings') {
    const current = await db.collection(CMS.settings).findOne({ _id: 'global' }) || { _id: 'global' };
    if (method === 'GET') return send(res, 200, { item: cleanDocument(current) });
    if (method === 'PUT') {
  ...
  if (parts[1] === 'reorder' && method === 'POST') {
    const input = await readJson(req, 256 * 1024);
    if (!Array.isArray(input.ids) || input.ids.length > 1000) return send(res, 400, { error: 'Danh sách sắp xếp không hợp lệ.' });
  ...
  try {
    ...
    if (method === 'POST') {
      ...
      await ensureEntityPage(db, action, value, inserted.insertedId);
    ...
    if (method === 'PUT' && id) {
      ...
      if (action === 'media') await replaceMediaReferences(db, current.url, value.url);
      await collection.updateOne({ _id: id }, { $set: value });
      await ensureEntityPage(db, action, value, id);
    ...
    if (method === 'DELETE' && id) {
      ...
      if (action === 'media' && current.publicId) await getCloudinary().uploader.destroy(current.publicId, { resource_type: 'image' });
    ...
    return send(res, 400, { error: error.message || 'Không xử lý được yêu cầu.' });
  }
}

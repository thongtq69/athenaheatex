...
import { checkDomainStatus } from './domain-status.mjs';
...
const validLink = value => /^https?:\/\/[^\s]+$/i.test(value) || /^\/(?!\/)[^\s]*$/.test(value) || /^mailto:[^\s]+$/i.test(value) || /^tel:[^\s]+$/i.test(value) || /^weixin:\/\/[^\s]+$/i.test(value);
...
function normalizeSettings(input = {}, current = {}) {
  ...
  const value = {
    ...
    address: text(input.address ?? current.address, 1000),
    whatsapp: text(input.whatsapp ?? current.whatsapp, 100), whatsappUrl: text(input.whatsappUrl ?? current.whatsappUrl, 2000),
    zalo: text(input.zalo ?? current.zalo, 100), zaloUrl: text(input.zaloUrl ?? current.zaloUrl, 2000),
    wechat: text(input.wechat ?? current.wechat, 100), wechatUrl: text(input.wechatUrl ?? current.wechatUrl, 2000),
  ...
  for (const [label, url] of [['WhatsApp', value.whatsappUrl], ['Zalo', value.zaloUrl], ['WeChat', value.wechatUrl]]) {
    if (url && !validLink(url)) throw new Error(`Liên kết ${label} không hợp lệ.`);
  }
...
export async function handleAdminApi(req, res, url, readJson) {
  ...
  if (action === 'domain-status' && method === 'GET') {
    return send(res, 200, await checkDomainStatus());
  }

  ...
  const run = async () => {
    ...
    for (let index = 0; index < files.length; index += 1) {
      ...
      if (isCategory) {
        ...
        categoryCandidates.push({ pathname, title, links, descriptionHtml: $('.content .info').first().html() || '' });
      }
      if (isProduct) {
    ...
    const services = servicePage ? [{
      ...
      descriptionHtml: sections.find(section => section.pagePath === servicePage.path)?.html || '', image: '',

...
const DEFAULT_FAVICON = '/favicon.ico?v=20260917-2';
const PUBLIC_SITE_ORIGIN = 'https://www.athenaheatex.com';
const SOCIAL_IMAGE_URL = `${PUBLIC_SITE_ORIGIN}/athena-heatex-share-preview-v2.jpg`;
const SOCIAL_IMAGE_ALT = 'ATHENA HEATEX - Complete Beverage Processing Solutions';
...
function setSocialMeta($, attribute, key, value) {
  $(`meta[${attribute}="${key}"]`).remove();
  $('head').append($('<meta>').attr(attribute, key).attr('content', value));
}

export function renderSocialMeta($, pathname = '/') {
  const title = String($('title').first().text() || 'ATHENA HEATEX').replace(/\s+/g, ' ').trim();
  const description = String($('meta[name="description"]').first().attr('content') || 'Giải pháp dây chuyền chế biến đồ uống hoàn chỉnh từ ATHENA HEATEX.').replace(/\s+/g, ' ').trim();
  const publicPath = publicPathForSourcePath(safePath(pathname));
  const pageUrl = new URL(publicPath, `${PUBLIC_SITE_ORIGIN}/`).href;

  setSocialMeta($, 'property', 'og:locale', 'vi_VN');
  setSocialMeta($, 'property', 'og:type', 'website');
  setSocialMeta($, 'property', 'og:site_name', 'ATHENA HEATEX');
  setSocialMeta($, 'property', 'og:title', title);
  setSocialMeta($, 'property', 'og:description', description);
  setSocialMeta($, 'property', 'og:url', pageUrl);
  setSocialMeta($, 'property', 'og:image', SOCIAL_IMAGE_URL);
  setSocialMeta($, 'property', 'og:image:secure_url', SOCIAL_IMAGE_URL);
  setSocialMeta($, 'property', 'og:image:type', 'image/jpeg');
  setSocialMeta($, 'property', 'og:image:width', '1200');
  setSocialMeta($, 'property', 'og:image:height', '630');
  setSocialMeta($, 'property', 'og:image:alt', SOCIAL_IMAGE_ALT);
  setSocialMeta($, 'name', 'twitter:card', 'summary_large_image');
  setSocialMeta($, 'name', 'twitter:title', title);
  setSocialMeta($, 'name', 'twitter:description', description);
  setSocialMeta($, 'name', 'twitter:image', SOCIAL_IMAGE_URL);
  setSocialMeta($, 'name', 'twitter:image:alt', SOCIAL_IMAGE_ALT);
}
...
export async function renderCmsPage(pathname) {
  ...
  if (normalized === '/index.html') $('title').text('ATHENA HEATEX');
  renderSocialMeta($, normalized);

...
function appendChannelLink($, list, channel, extraClass = '') {
  const anchor = $('<a>').attr({ href: channel.url, 'aria-label': `Liên hệ qua ${channel.label}`, title: `${channel.label}: ${channel.value}` });
  ...
  list.append($('<li>').addClass(`contact-channel-item ${extraClass}`).append(anchor));
}
...
export function renderContact($, settings = ATHENA_CONTACT_DEFAULTS) {
  ...
  $('.crm-form').each((_, node) => {
    const form = $(node);
    form.children('.form-contact-channels').remove();
    const block = $('<div class="form-contact-channels" aria-label="Liên hệ trực tiếp">');
    const list = $('<ul>');
    for (const channel of channels) appendChannelLink($, list, channel, 'form-contact-channel');
    if (email) {
      const channel = { kind: 'email', label: 'Email', value: email, url: `mailto:${email}` };
      const anchor = $('<a>').attr({ href: channel.url, 'aria-label': 'Liên hệ qua Email', title: email });
      anchor.append('<span class="contact-channel-icon contact-channel-email" aria-hidden="true">@</span><span class="contact-channel-label">Email</span>');
      list.append($('<li class="contact-channel-item form-contact-channel">').append(anchor));
    }
    if (list.children().length) {
      block.append($('<p>').text('Liên hệ trực tiếp')).append(list);
      const title = form.find('.create-form-title').first();
      if (title.length) title.after(block); else form.prepend(block);
    }
  });
  ...
  if (mobileBar.length) {
    ...
    for (const channel of channels.slice(0, 3)) {
      const item = $('<li>');
      const anchor = $('<a>').attr({ href: channel.url, 'aria-label': channel.label, title: channel.value });

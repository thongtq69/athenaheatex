...
export async function notifyByEmail(record, id, configuredRecipient = '') {
  // This mailbox owns the activated FormSubmit endpoint and must receive every
  // website inquiry even if a public contact address is edited in the CMS.
  const to = DEFAULT_NOTIFICATION_EMAIL;
...
export async function submitInquiry(input, { address = '', userAgent = '', dataDir, useDatabase = isDatabaseConfigured(), now = new Date() } = {}) {
  ...
  try {
    ...
    // Any server-side delivery failure falls back to the activated FormSubmit
    // endpoint in the browser. A one-time token lets the browser report the
    // final outcome without allowing arbitrary inquiry updates.
    const needsClientNotification = notification.status !== 'sent';
    const notificationToken = needsClientNotification ? randomUUID() : null;
    try {
      await collection.updateOne({ _id: insertedId }, { $set: {
        notificationStatus: needsClientNotification ? 'client_required' : notification.status,
        notificationProvider: needsClientNotification ? 'formsubmit' : notification.provider,
        notificationRecipient: DEFAULT_NOTIFICATION_EMAIL,
    ...
    const finalNotification = needsClientNotification ? 'client_required' : notification.status;
    const message = finalNotification === 'sent' ? MESSAGES.saved : MESSAGES.savedWithoutNotification;
    return { status: 201, body: { ok: true, id, delivery: 'database', notification: finalNotification,
      ...(notificationToken ? { notificationToken, notificationRecipient: DEFAULT_NOTIFICATION_EMAIL } : {}), message } };
  } catch (failure) {
